defmodule LeafNodeWeb.Api.DocumentController do
  ...
  def get_documents_list(conn, _params) do
    case LeafNode.Node.list_documents do
      {:ok, data} ->
  ...
  def get_document_by_id(conn, %{ "id" => id}) do
    case LeafNode.Node.get_document(id) do
      {:ok, data} ->
  ...
  def update_document(conn, params) do
    case LeafNode.Node.edit_document(params) do
      {:ok, data} ->
        return(conn, 200, Helpers.http_resp(200, true, data))
      {:error, err} ->
        return(conn, 404, Helpers.http_resp(404, false, err))
 ...
 @doc """
  ...
  def create_document(conn, _params) do
    case LeafNode.Node.create_document() do
      {:ok, data} ->
        return(conn, 200, Helpers.http_resp(200, true, data))
      {:error, err} ->
        return(conn, 404, Helpers.http_resp(404, false, err))
 ...
 @doc """
  ...
  def delete_document(conn, %{ "id" => id}) do
    case LeafNode.Node.delete_document(id) do
      {:ok, data} ->
...
@doc """
    Execute document - verbose - we return logs around each of the text blocks that ran
  """
